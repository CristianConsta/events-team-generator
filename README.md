...
## Migration Script (Promote Event Building Defaults)

Use this one-time script to copy building names / `#Players` / priorities from a source user event config and apply them to:

- shared defaults doc: `app_config/default_event_building_config`
- all existing user docs: `users/*` for the selected event

Default source/event values:

- source email: `constantinescu.cristian@gmail.com`
- event id: `canyon_battlefield`

1. Dry run:

```bash
node scripts/sync_event_building_defaults.js --service-account PATH_TO_SERVICE_ACCOUNT.json --project-id YOUR_PROJECT_ID
```
...
```bash
node scripts/sync_event_building_defaults.js --service-account PATH_TO_SERVICE_ACCOUNT.json --project-id YOUR_PROJECT_ID --apply --source-email SOMEONE@EXAMPLE.COM --event-id canyon_battlefield
```
